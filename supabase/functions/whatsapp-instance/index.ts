...
Deno.serve(async (req) => {
  ...
  const providerResponse = await fetch(n8nUrl, { method: "POST", headers: { "Content-Type": "application/json", "x-internal-secret": Deno.env.get("N8N_SHARED_SECRET") ?? "" }, body: JSON.stringify({ action, organization_id: member.organization_id, instance_name: payload.instance_name }) });
  ...
  if (action === "create") {
    await supabase.from("channel_connections").upsert({ organization_id: member.organization_id, provider: "n8n_uazapi", name: payload.instance_name ?? "WhatsApp", external_instance_id: providerPayload.instanceId, status: "pairing", webhook_secret_encrypted: providerPayload.webhookSecret ? "managed-server-side" : null }, { onConflict: "organization_id,provider,external_instance_id" });
  }
