...
CREATE TABLE IF NOT EXISTS public.organization_settings (
  ...
  default_appointment_interval integer NOT NULL DEFAULT 30,
  minimum_booking_notice_minutes integer NOT NULL DEFAULT 120,
  maximum_booking_days_ahead integer NOT NULL DEFAULT 30,
...
CREATE TABLE IF NOT EXISTS public.customers (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  full_name text NOT NULL DEFAULT 'Cliente WhatsApp',
...
CREATE TABLE IF NOT EXISTS public.channel_connections (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  provider text NOT NULL CHECK (provider IN ('mock','uazapi','meta_cloud_api','n8n_uazapi')),
  name text NOT NULL,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (organization_id, provider, external_instance_id)
);
...
CREATE TABLE IF NOT EXISTS public.appointments (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  EXCLUDE USING gist (organization_id WITH =, professional_id WITH =, tstzrange(start_at, end_at, '[)') WITH &&)
...
CREATE TABLE IF NOT EXISTS public.ai_settings (
  ...
  max_tool_steps integer NOT NULL DEFAULT 5,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.webhook_events (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (channel_connection_id, provider, external_event_id)
);
...
CREATE INDEX IF NOT EXISTS idx_appointments_org_start ON public.appointments(organization_id, start_at);
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'organization_settings','onboarding_progress','locations','services','professionals',
    'professional_services','availability_rules','customers','channel_connections',
    'conversations','messages','appointments','ai_settings','webhook_events'
  ] LOOP
    EXECUTE format('DROP POLICY IF EXISTS tenant_all ON public.%I', table_name);
    EXECUTE format('CREATE POLICY tenant_all ON public.%I FOR ALL TO authenticated USING (public.is_org_member(organization_id)) WITH CHECK (public.is_org_member(organization_id))', table_name);
  END LOOP;
END $$;
