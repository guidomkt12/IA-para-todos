...
export async function createOrganization(input: { name: string; slug: string; business_type: string; phone?: string; email?: string }) {
  ...
  const { data: org, error } = await db.from("organizations").insert(input).select("*").single();
  if (error) throw error;
  const member = { organization_id: org.id, user_id: userResult.user.id, role: "owner", status: "active" };
  const { error: memberError } = await db.from("organization_members").insert(member);
  if (memberError) throw memberError;
  await db.from("organization_settings").insert({ organization_id: org.id });
  await db.from("onboarding_progress").insert({ organization_id: org.id, current_step: "unidade", completed_steps: ["empresa"] });
  return org;
}
...
export async function loadDashboardData(organizationId: string) {
  ...
  for (const result of [appointments, customers, conversations, services, professionals, connections, progress]) if (result.error) throw result.error;
  return { appointments: appointments.data ?? [], customers: customers.data ?? [], conversations: conversations.data ?? [], messagesInbound: inbound.count ?? 0, messagesOutbound: outbound.count ?? 0, services: services.data ?? [], professionals: professionals.data ?? [], connection: connections.data?.[0], progress: progress.data };
}
...
export async function createProfessional(organizationId: string, input: { display_name: string; email?: string; phone?: string; service_ids?: string[] }) {
  ...
  if (error) throw error;
  if (input.service_ids?.length) await db.from("professional_services").insert(input.service_ids.map((service_id) => ({ organization_id: organizationId, professional_id: data.id, service_id })));
  return data;
}
